...
public static class MenuSettings
{
    ...
    public static event System.Action<Zone>? ZoneChanged;
    ...
    /*/// <summary>
    /// Get all configured hazards across all zones (legacy compatibility)
    /// </summary>
    /// <returns>Dictionary of (Zone, SubZoneArea, SpawnType, int) to HazardSettings</returns>
    public static Dictionary<(Zone, SubZoneArea, HazardType, int), HazardSettings> GetAllHazards()
    {
        var result = new Dictionary<(Zone, SubZoneArea, HazardType, int), HazardSettings>();
        int index = 1;
        
        foreach (var hazard in _hazardData.Values)
        {
            var key = (hazard.Zone, hazard.SubZoneArea, SpawnType: hazard.hazardType, index);
            result[key] = new HazardSettings(hazard.Amount);
            index++;
        }
